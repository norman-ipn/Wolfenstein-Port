 ...
 *  \filename id_vl.c
 *
 *  new version of ID_VL.C
 *
 *  NOTE: lot of errors to fix .....
 *
 * */

#include "id_vl.h"  /*<< all the includes needes were moved here */
...
We are not going to use this approach of compiling.
All .c files must have a .h file (protected by centinels) 
and then we generate the .o file (the libraries).
Then link them in a final a.out (main binary file)
*/

 // #pragma hdrstop  /* not working with that model */
 ...
 *
 * */
unsigned int bufferofs = 0;
unsigned int displayofs = 0;
unsigned int pelpan = 0;

unsigned int screenseg = SCREENSEG;		// set to 0xa000 for asm convenience

unsigned int linewidth = 0;
unsigned int ylookup[MAXSCANLINES];
...
/*
 * This function select the video card, but we are only working with GTK
 * so GTK selects the video card, 
 *
 * We don't need this fuction 
 *
 */
void	VL_Startup (void)
{
...
//	int videocard = 0;
	...
	// asm	cld;
	/* we are not using te direction flag, so just comment it */
        /*
...
unsigned char pixmasks[4] = {1,2,4,8};
unsigned char leftmasks[4] = {15,14,12,8};
unsigned char rightmasks[4] = {1,3,7,15};
...
{
	unsigned int xbyte = 0;
	unsigned char *dest = NULL;
	unsigned char leftmask = 0;
	unsigned char rightmask = 0;
	int midbytes = 0;
