...
/*
  ...
  Drawing a String on the screen.
	...
	Uses:
		...
		global variables:
			fontnumber, to get the font pointer.
			px and py, (px,py) position of .... something....
...
/* 
  ...
  Also for drawing string on the screen, but on color.

  A function can handle both options: color and no color.
...
= VL_MungePic
   ...
   Is like a decription process fot the pictures used on the game.

   Not needed because, the images will be generated by our 3D libraries.
...
-----------------------------TEMPORAL COMMENT----------------------------------
	...
	byte		*temp, far *dest, far *srcline;
...
/**
   These three functions are used as auxiliar code to draw texts,
 
   Text drawing will be done with cairo.

   We do'nt need thesse functions.  
*/
...
=  
  ...
  This functions is useful to speed the game, 
  because just paint the changes on the screen.
  
  A similar function is avaiable on cairo.
  The function marks areas as  dirty in order to be painted. 
...
/**
   ...
   There are M and not M functions, 
   we need to figure out what is the difference.   
